...
bool is_identical(const WMSLayerHierarchy& lh1, const WMSLayerHierarchy& lh2, WMSLayerHierarchy::ElementType type)
{
  ...
  if(type == WMSLayerHierarchy::ElementType::geo_bbox)
	{
	  ...
	  if((layer1 && layer2 && layer1->identicalGeographicBoundingBox(*layer2)) || (!layer1 && !layer2))
		return true;
	  else
		return false;
	}
  else  if(type == WMSLayerHierarchy::ElementType::proj_bbox)
	{
	  ...
	  if((layer1 && layer2 && layer1->identicalProjectedBoundingBox(*layer2)) || (!layer1 && !layer2))
		return true;
	  else
		return false;
	}
...
void expand_layer(WMSLayerHierarchy& lh)
{
  if(lh.timeDimension)
	{
	  const boost::shared_ptr<WMSTimeDimensions>& td = lh.timeDimension->getLayer()->getTimeDimensions();
	  if(td)
		{
...
void add_sublayers(WMSLayerHierarchy& lh, 
				   std::map<std::string, 
				   const WMSLayerProxy*> named_layers, 
					...
					unsigned int level) 
{
	...
	bool sublayer_is_reference_time_layer = (lh.sublayers.size() > 0 && lh.sublayers.back()->reference_time);
	...
	if(baseInfo)
	  ...
	  {
		// From sepc: "If the layer has a Title but no Name, then that layer is only a category title for all the layers nested within."
		capa["title"] = lh.name;
		capa["queryable"] = 0;
	  }
	...
	if(lh.geographicBoundingBox)
	  {
		if(!lh.parent || (lh.parent && !is_identical(lh, *lh.parent, WMSLayerHierarchy::ElementType::geo_bbox)))
		  {
			// Add supported geographic bounding box
			boost::optional<CTPP::CDT> geo_bbox =  lh.geographicBoundingBox->getLayer()->getGeographicBoundingBoxInfo();
			if(geo_bbox)
	...
	if(lh.projectedBoundingBox)
	  {
		if(!lh.parent || (lh.parent && !is_identical(lh, *lh.parent, WMSLayerHierarchy::ElementType::proj_bbox)))
		  {
			// Add supported CRSs
			boost::optional<CTPP::CDT> projected_bbox =  lh.projectedBoundingBox->getLayer()->getProjectedBoundingBoxInfo();
			if(projected_bbox)
	...
	if (lh.timeDimension)
	  {
			...
			if(sublayer_is_reference_time_layer)
			  ...
			  {
				if(!lh.parent || (lh.parent && !is_identical(lh, *lh.parent, WMSLayerHierarchy::ElementType::time_dim)))
				  time_dim =  lh.timeDimension->getLayer()->getTimeDimensionInfo(starttime, endtime, reference_time);
			  }
	...
	if(lh.elevationDimension)
	  {
		if(!lh.parent || (lh.parent && !is_identical(lh, *lh.parent, WMSLayerHierarchy::ElementType::elev_dim)))
		  {
			// Add supported elevation dimension
			boost::optional<CTPP::CDT> elev_dim =  lh.elevationDimension->getLayer()->getElevationDimensionInfo();
			if(elev_dim)
...
WMSLayerHierarchy::WMSLayerHierarchy(const std::map<std::string, WMSLayerProxy>& layerMap, 
									 ...
									 Engine::Authentication::Engine* authEngine) : name("__root__")
#else
WMSLayerHierarchy::WMSLayerHierarchy(const std::map<std::string, WMSLayerProxy>& layerMap, const boost::optional<std::string>& wms_namespace, HierarchyType hierarchy_type) : name("__root__")
#endif
{
#ifndef WITHOUT_AUTHENTICATION
...
CTPP::CDT WMSLayerHierarchy::getCapabilities(const boost::optional<std::string>& starttime,
											 const boost::optional<std::string>& endtime,
...
void printHierarchy(const std::string& prefix, unsigned int level, std::ostream& ost, const WMSLayerHierarchy& lh)
{
  ost << prefix << lh.name << "(L#" << level << (lh.sublayers.empty() ? "-Leaf)" : ") ") 
	  << (lh.geographicBoundingBox ? "1" : "0") 
	  << (lh.projectedBoundingBox ? "1" : "0") 
	  << (lh.timeDimension ? "1" : "0")
	  << (lh.elevationDimension ? "1" : "0") << std::endl;
