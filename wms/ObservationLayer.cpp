  ...
  try
  {
    ...
    if (keyword.empty() && fmisids.empty())
      throw Fmi::Exception::Trace(
          BCP, "Either 'keyword' or 'fmisid' must be defined for observation layer! ");
  }
  catch (...)
  {
...
ResultSet ObservationLayer::getObservations(State& theState,
                                            ...
                                            const boost::posix_time::ptime& endtime) const
{
  try
  {
    ...
    settings.timestep =
        0;  // All timesteps are fetched, after postprocessing only requested timestep(s) are used
    ...
    for (const auto& item : fmisid_ranges)
      ret.insert(
          std::make_pair(item.first,
                         std::vector<TS::TimeSeries>(settings.parameters.size(),
                                                     TS::TimeSeries(settings.localTimePool))));
...
ResultSet ObservationLayer::getObservations(
    State& theState, const boost::posix_time::ptime& requested_timestep) const
{
  try
  {
...
void ObservationLayer::generate(CTPP::CDT& theGlobals, CTPP::CDT& theLayersCdt, State& theState)
{
  try
  {
    if (theState.getConfig().obsEngineDisabled())
      throw std::runtime_error(
          "Cannot use PresentWeatherObservationLayer when the observation engine is disabled");
  ...
  catch (...)
  {
    throw Fmi::Exception::Trace(BCP,
                                "Generating template hash for finnish road observations failed!");
  }
}
