...
PostGISLayer::PostGISLayer(const LayerConfig& config, Json::Value& json)
    : Layer(config), itsGisEngine(config.gisEngine()), hasTemporalDimension(false)
{
  try
  {
    ...
    itsMetaDataSettings = get_postgis_metadata_settings(
        config.getDaliConfig().getConfig(), mdq_options.pgname, mdq_options.schema);
...
bool PostGISLayer::mustUpdateLayerMetaData()
{
  try
  {
  ...
  {
    throw Fmi::Exception(
        BCP, "PostGISLayer::mustUpdateLayerMetaData() function call failed!", nullptr);
  }
}

bool PostGISLayer::updateLayerMetaData()
{
  try
  {
    ...
    if (hasTemporalDimension && !timeDimensionDisabled)
    {
      ...
      std::shared_ptr<TimeDimension> timeDimension = nullptr;
