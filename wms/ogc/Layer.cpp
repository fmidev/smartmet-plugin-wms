...
namespace OGC
{
using WMS::CaseInsensitiveComparator;
namespace
{
...
void add_symbol_name(Json::Value& json,
                     const LegendGraphicSettings& lgs,
  ...
  if (!layersJson.isNull() && layersJson.isArray())
  {
    ...
    for (auto& layerJson : layersJson)
    {
      if (!layerJson.isNull())
      {
        ...
        for (auto& subLayerJson : subLayersJson)
        {
  ...
  for (const auto& layerJson : layersJson)
  {
    if (!layerJson.isNull())
    {
      ...
      if (!layerTypeJson.isNull())
      {
          ...
          if (json.isArray())
          {
            for (const auto& isoband : json)
            {
              ...
              if (!label.isNull())
              {
                ...
                for (const auto& language : members)
                {
        ...
        {
          // Expand symbol group
          Json::Value symbolGroup = Layer::parseJsonString(json.toStyledString());
          if (symbolGroup.isArray())
          {
            for (const auto& symbolJson : symbolGroup)
            {
...
void get_legend_graphic_settings(const Json::Value& root, LegendGraphicSettings& settings)
{
  Json::Value nulljson;
...
std::map<std::string, LayerStyle> get_styles(const Json::Value& root,
                                                const std::string& layerName,
                                                std::map<std::string, std::string>& legendFiles,
                                                const LayerConfig& config)
{
  std::map<std::string, LayerStyle> ret;
  Json::Value nulljson;
  ...
  {
    if (!json.isArray())
      throw Fmi::Exception(BCP, "Layer styles settings must be an array");
    ...
    for (const auto& style_json : json)
    {
      ...
      if (json_value.isNull())
        throw Fmi::Exception(BCP, "Layer styles must have a name");
...
NamedLegendGraphicInfo get_named_info(const Json::Value& root,
                                      ...
                                      const std::map<std::string, LayerStyle>& styles,
...
Layer::Layer(const LayerConfig& lc) : queryable(true), config(lc)
{
  config.getDaliConfig().getConfig().lookupValue("wms.get_capabilities.update_interval",
...
void Layer::initLegendGraphicInfo(const Json::Value& root)
{
  Json::Value nulljson;
  ...
  // 1) Default settings
  LegendGraphicSettings actualSettings(true);  // Default values

  // 2) Settings from wms.conf
  LegendGraphicSettings configSettings = config.getLegendGraphicSettings();
  actualSettings.merge(configSettings);

  // 3) Settings from layer file (product file)
  LegendGraphicSettings layerSettings;
  get_legend_graphic_settings(root, layerSettings);

  // Enable languages listed in layers in addition to configured ones
  std::set<std::string> languages = config.getDaliConfig().languages();
  ...
  std::map<std::string, Json::Value> templateFiles = readLegendFiles(
      config.getDaliConfig().rootDirectory(true), config.getJsonCache(), customer);
  std::map<std::string, LegendGraphicSettings> templateSettings;

  for (const auto& item : templateFiles)
  {
    LegendGraphicSettings settings;
  ...
  // Get layer styles
  itsStyles = get_styles(root, name, itsLegendFiles, config);
  ...
  for (const auto& language : languages)
  {
    for (const auto& item : named_lgi)
    {
      ...
      for (const auto& lgi : legendGraphicInfo)
      {
        ...
        // 3) Settings from template file will overwrite the previous settings (default settings,
        // wms.conf)
        const auto& templateJson = templateFiles.at(key);
        actualSettings.merge(templateSettings.at(key));

        // 4) Settings from layer file (product file) will overwrite the previous settings (default
        // settings, wms.conf, template file)
        ...
        if (layerType == "isoband")
        {
          std::string customerRoot =
              (config.getDaliConfig().rootDirectory(true) + "/customers/" + customer);
          std::string layersRoot = customerRoot + "/layers/";

          Spine::JSON::preprocess(legendJson,
                                  config.getDaliConfig().rootDirectory(true),
                                  layersRoot,
                                  config.getJsonCache());
      ...
      if (!lgiSymbols.empty())
      {
        for (const auto& symbol : lgiSymbols)
        {
          std::string templateName = Fmi::ascii_tolower_copy(symbol.first + "_symbol");
          if (symbol.first.empty() || templateFiles.find(templateName) == templateFiles.end())
            templateName = "symbol";

          LegendGraphicSettings actualSymbolSettings = actualSettings;
          // 3) Settings from template file will overwrite the previous settings (default settings,
          // wms.conf)
          actualSymbolSettings.merge(templateSettings.at(templateName));
          // 4) Settings from layer file (product file) will overwrite the previous settings
          // (default settings, wms.conf, template file)
          actualSymbolSettings.merge(layerSettings);

          Json::Value symbolTemplate = templateFiles.at(templateName);
          const LegendGraphicInfo& legendGraphicInfo = symbol.second;
  ...
  for (const auto& item : itsLegendGraphicResults)
  {
    ...
    {
      if (item2.first == config.getDaliConfig().defaultLanguage())
      {
        const LegendGraphicResult& result =
            languageResult.at(config.getDaliConfig().defaultLanguage());
...
LegendGraphicResult Layer::getLegendGraphic(const std::string& legendGraphicID,
                                               const std::string& language) const
{
  if (itsLegendGraphicResults.find(legendGraphicID) != itsLegendGraphicResults.end())
  {
    ...
    return layerLGR.at(config.getDaliConfig().defaultLanguage());
  }

  return {};
}

bool Layer::isValidInterval(int interval_start, int interval_end) const
{
  if (!intervalDimension)
    return false;

  return intervalDimension->isValidInterval(interval_start, interval_end);
}

bool Layer::isValidCRS(const std::string& theCRS) const
{
  try
  {
    auto crs = theCRS;
...
bool Layer::isValidStyle(const std::string& theStyle) const
{
  try
  {
...
bool Layer::isValidElevation(int theElevation) const
{
  try
  {
...
bool Layer::isValidReferenceTime(const Fmi::DateTime& theReferenceTime) const
{
  try
  {
    if (timeDimensions)
...
bool Layer::isValidTime(const Fmi::DateTime& theTime,
...
Fmi::DateTime Layer::mostCurrentTime(const std::optional<Fmi::DateTime>& reference_time) const
{
  try
  {
...
std::optional<CTPP::CDT> Layer::getLayerBaseInfo(const std::string& language,
...
std::optional<CTPP::CDT> Layer::getGeographicBoundingBoxInfo() const
{
  try
  {
    if (hidden)
...
std::optional<CTPP::CDT> Layer::getProjectedBoundingBoxInfo() const
{
  try
  {
    if (hidden)
...
Layer::getLimitedCapabilitiesInterval(const std::optional<Fmi::DateTime>& starttime,
...
std::optional<CTPP::CDT> Layer::getTimeDimensionInfo(
  ...
  try
  {
    ...
    if (timeDimensions)
    {
      if (reference_time)
      {
        ...
        {
          const TimeDimension& td = timeDimensions->getTimeDimension(*reference_time);
      ...
      {
        const TimeDimension& td = timeDimensions->getDefaultTimeDimension();
        auto dim_string = td.getCapabilities(multiple_intervals, time1, time2);
        if (dim_string.empty())
          return {};
...
std::optional<CTPP::CDT> Layer::getIntervalDimensionInfo() const
{
  try
  {
...
std::optional<CTPP::CDT> Layer::getReferenceDimensionInfo() const
{
  try
  {
    if (hidden)
...
std::optional<CTPP::CDT> Layer::getElevationDimensionInfo() const
{
  try
  {
    if (hidden)
...
std::optional<CTPP::CDT> Layer::getStyleInfo(const std::string& language,
                                                const std::string& defaultLanguage) const
{
  try
  {
...
const std::shared_ptr<TimeDimensions>& Layer::getTimeDimensions() const
{
  return timeDimensions;
}

std::optional<CTPP::CDT> Layer::generateGetCapabilities(
    bool multiple_intervals,
    bool show_hidden,
...
Json::Value Layer::parseJsonString(const std::string& theJsonString)
{
  Json::Value json;
...
void Layer::setCustomer(const std::string& c)
{
  customer = c;
...
void Layer::setDefaultElevation(int level)
{
  if (elevationDimension)
    elevationDimension->setDefaultElevation(level);
}

bool Layer::identicalGeographicBoundingBox(const Layer& layer) const
{
  return geographicBoundingBox == layer.geographicBoundingBox;
}

bool Layer::identicalProjectedBoundingBox(const Layer& layer) const
{
...
bool Layer::identicalTimeDimension(const Layer& layer) const
{
...
bool Layer::identicalElevationDimension(const Layer& layer) const
{
  // If both are nullptr -> identical
...
bool Layer::identicalKeywords(const Layer& layer) const
{
...
std::optional<CTPP::CDT> Layer::getProjectedBoundingBoxDelta(const Layer& inherited) const
{
  try
  {
    if (hidden)
...
void Layer::setLegendDimension(const Layer& legendLayer, const std::string& styleLayerName)
{
...
void Layer::setProductFile(const std::string& theProductFile)
{
...
const Fmi::DateTime& Layer::modificationTime() const
{
  return itsProductFileModificationTime;
}

void Layer::addIntervalDimension(int interval_start, int interval_end, bool interval_default)
{
  if (!intervalDimension)
    intervalDimension =
        std::make_shared<IntervalDimension>(interval_start, interval_end, interval_default);
  else
    intervalDimension->addInterval(interval_start, interval_end, interval_default);
}

std::pair<std::string, std::string> Layer::getDefaultInterval() const
{
