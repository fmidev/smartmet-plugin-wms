...
std::string RasterLayer::generateGeoTiff(State& theState)
{
  try
  {
    ...
    if (paraminfo.parameter.empty())
      throw Fmi::Exception(BCP, "Parameter not set for geotiff layer");

    if (!paraminfo.producer)
      throw Fmi::Exception(BCP, "Producer not set for geotiff layer");

    if (!projection.crs || *projection.crs == "data")
      throw Fmi::Exception(BCP,
                           "GeoTiff output requires an explicit CRS (crs=data is not supported)");

    // ---- Build the grid query (mirrors generate_gridEngine) ----
    ...
    const auto& final_box = projection.getBox();
    double gt[6] = {
        final_box.xmin(),
