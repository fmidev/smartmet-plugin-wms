...
public:
  ...
  void setMeanDeviation(double mean) { itsMeanDeviation = abs(mean - itsPointValueBase->priorityValue()); }
  ...
  try
	{
	  ...
	  if(opts.priority)
	  ...
	  else if(opts.priorities)
		{
		  ...
		  prioritized_points.insert(std::make_pair(non_prioritized_category, std::vector<PointValueWrapper>()));
		  
		  for(auto & item : points)
			{
			  // Store as integer to get the right category
			  auto prio_category = item.priorityValue();
			  auto map_index = (priority_categories.count(prio_category) > 0 ? prio_category : non_prioritized_category);
...
std::vector<WindArrowValue> prioritize(const std::vector<WindArrowValue>& pv, const PointValueOptions& opts)
{
  return prioritize_t(pv, opts);
}
