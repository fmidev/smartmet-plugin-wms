...
bool prepareLegendGraphic(Product& theProduct)
{
  ...
  std::shared_ptr<Dali::Layer> legendLayer;

  bool legendLayerFound = false;
  for (const auto& view : views)
  {
    std::list<std::shared_ptr<Dali::Layer>> layers = view->layers.layers;

    for (const auto& layer : layers)
    {
      std::list<std::shared_ptr<Dali::Layer>> sublayers = layer->layers.layers;
      ...
      for (const auto& sublayer : sublayers)
        std::list<std::shared_ptr<Dali::Layer>> sublayers2 = sublayer->layers.layers;
    }
    if (legendLayerFound)
      break;
  }
...
void update_capabilities_modification_time(Fmi::DateTime& mod_time, const SharedLayer& layer)
{
...
CTPP::CDT Config::get_capabilities(const libconfig::Config& config) const
{
...
void Config::parse_references()
{
  ...
  for (int i = 0; i < epsg_settings.getLength(); i++)
  {
    ...
    SupportedReference ref(name, bbox, enabled, geographic);
    itsSupportedReferences.insert(std::make_pair(name, ref));
  }
  ...
  for (int i = 0; i < crs_settings.getLength(); i++)
  {
    ...
    SupportedReference ref(proj, bbox, enabled, geographic);
    itsSupportedReferences.insert(std::make_pair(name, ref));
  }
...
Config::Config(const Dali::Config& daliConfig,
      ...
      itsLegendGraphicSettings(daliConfig.getConfig())
{
  try
  {
    ...
    if (layout == "flat")
      itsLayerHierarchyType = LayerHierarchy::HierarchyType::flat;
    else if (layout == "recursive")
      itsLayerHierarchyType = LayerHierarchy::HierarchyType::recursive;
    else if (layout == "recursivetimes")
      itsLayerHierarchyType = LayerHierarchy::HierarchyType::recursivetimes;
    else
...
Config::~Config()
{
  ...
  {
    std::cout << "ERROR [WMS][Config]: Missing call to Config::shutdown(). Terminating...\n";
    abort();
  }
}
...
void Config::capabilitiesUpdateLoop()
{
  try
  {
...
#ifndef WITHOUT_AUTHENTICATION
bool Config::validateGetMapAuthorization(const Spine::HTTP::Request& theRequest) const
{
...
void Config::updateLayerMetaDataForCustomerLayer(
    ...
    LayerMap& newProxies,
    std::map<SharedLayer, std::map<std::string, std::string>>& externalLegends)
{
  ...
  try
  {
    ...
    if (mylayers && mylayers->find(fullLayername) != mylayers->end())
    {
      ...
      SharedLayer oldLayer = oldProxy.getLayer();
      ...
      else
      {
        for (const auto& newlayer : newlayers)
        {
          if (newlayer)
          {
            ...
            LayerProxy newProxy(itsGisEngine, newlayer);
...
void Config::updateLayerMetaDataForCustomer(
...
void Config::updateLayerMetaData()
{
  try
  {
...
#ifndef WITHOUT_AUTHENTICATION
CTPP::CDT Config::getCapabilities(const std::optional<std::string>& apikey,
  ...
  try
  {
    ...
    if (hierarchy_type != LayerHierarchy::HierarchyType::flat)
    {
      ...
      LayerHierarchy lh(*my_layers,
                           ...
                           itsAuthEngine);
#else
      LayerHierarchy lh(*my_layers, wms_namespace, hierarchy_type, show_hidden);
#endif
...
std::string Config::layerCustomer(const std::string& theLayerName) const
{
  try
  {
    auto my_layers = itsLayers.load();
...
const std::set<std::string>& Config::supportedMapFormats() const
{
  return itsSupportedMapFormats;
}
const std::set<std::string>& Config::supportedWMSVersions() const
{
  return itsSupportedWMSVersions;
}
const std::map<std::string, SupportedReference>& Config::supportedWMSReferences() const
{
  return itsSupportedReferences;
}
const std::set<std::string>& Config::supportedWMSExceptions() const
{
  return itsSupportedWMSExceptions;
}
const std::set<std::string>& Config::supportedWMSGetCapabilityFormats() const
{
  return itsSupportedWMSGetCapabilityFormats;
}

bool Config::isValidMapFormat(const std::string& theMapFormat) const
{
...
bool Config::isValidVersion(const std::string& theVersion) const
{
  return (itsSupportedWMSVersions.find(theVersion) != itsSupportedWMSVersions.end());
}

bool Config::isValidLayer(const std::string& theLayer,
...
bool Config::isValidStyle(const std::string& theLayer, const std::string& theStyle) const
{
  try
  {
    ...
    SharedLayer layer = my_layers->at(theLayer).getLayer();
...
bool Config::isValidInterval(const std::string& theLayer,
                                int interval_start,
...
std::pair<std::string, std::string> Config::getDefaultInterval(const std::string& theLayer) const
{
  try
  {
...
std::string Config::getCRSDefinition(const std::string& theCRS) const
{
  try
  {
    // Normal EPSG and named references
    const auto pos = itsSupportedReferences.find(theCRS);
    if (pos != itsSupportedReferences.end())
...
bool Config::isValidCRS(const std::string& theLayer, const std::string& theCRS) const
{
  try
  {
...
bool Config::isValidElevation(const std::string& theLayer, int theElevation) const
{
  try
  {
    if (!isValidLayerImpl(theLayer))
...
bool Config::isValidReferenceTime(const std::string& theLayer,
...
bool Config::isValidTime(const std::string& theLayer,
...
bool Config::isTemporal(const std::string& theLayer) const
{
  try
  {
    if (!isValidLayerImpl(theLayer))
...
bool Config::currentValue(const std::string& theLayer) const
{
  try
  {
    if (!isValidLayerImpl(theLayer))
...
Fmi::DateTime Config::mostCurrentTime(const std::string& theLayer,
...
SharedLayer Config::getLayer(const std::string& theLayer) const
{
  try
  {
    if (!isValidLayerImpl(theLayer))
...
Json::Value Config::json(const std::string& theLayerName) const
{
  try
  {
    auto my_layers = itsLayers.load();
...
std::vector<Json::Value> Config::getLegendGraphic(const std::string& layerName,
...
bool Config::isValidLayerImpl(const std::string& theLayer,
                                 bool theAcceptHiddenLayerFlag /*= false*/) const
{
  try
  {
    ...
    LayerProxy lp = my_layers->at(theLayer);
...
bool Config::inspireExtensionSupported() const
{
  return itsInspireExtensionSupported;
}

CTPP::CDT Config::getCapabilitiesResponseVariables() const
{
  return get_capabilities(itsDaliConfig.getConfig());
}

#ifndef WITHOUT_OBSERVATION
std::set<std::string> Config::getObservationProducers() const
{
...
void Config::getLegendGraphic(const std::string& theLayerName,
                                 ...
                                 const std::string& theLanguage) const
{
  ...
  {
    std::shared_ptr<Dali::Layer> legendLayer(Dali::LayerFactory::create(legendL));
    ...
    if (!defs.isNull())
    {
      ...
      if (!defLayers.isNull())
      {
        for (auto& defLayerJson : defLayers)
        {
          ...
          {
            std::shared_ptr<Dali::Layer> defLayer(Dali::LayerFactory::create(defLayerJson));
...
const LegendGraphicSettings& Config::getLegendGraphicSettings() const
{
