...
bool TimeDimension::isValidTime(const Fmi::DateTime& theTime,
...
std::set<Fmi::DateTime> TimeDimension::getTimeSteps() const
{
  return itsTimesteps;
}
...
TimeDimensions::TimeDimensions(
    const std::map<Fmi::DateTime, std::shared_ptr<TimeDimension>>& tdims)
{
  for (const auto& td : tdims)
    addTimeDimension(td.first, td.second);
}

void TimeDimensions::addTimeDimension(const Fmi::DateTime& origintime,
                                         const std::shared_ptr<TimeDimension>& td)
{
...
const TimeDimension& TimeDimensions::getDefaultTimeDimension() const
{
  return *itsTimeDimensions.at(itsDefaultOrigintime);
}

const TimeDimension& TimeDimensions::getTimeDimension(const Fmi::DateTime& origintime) const
{
  return *itsTimeDimensions.at(origintime);
}

bool TimeDimensions::origintimeOK(const Fmi::DateTime& origintime) const
{
  return itsTimeDimensions.find(origintime) != itsTimeDimensions.end();
}

const std::vector<Fmi::DateTime>& TimeDimensions::getOrigintimes() const
{
  return itsOrigintimes;
}

bool TimeDimensions::isValidReferenceTime(const Fmi::DateTime& origintime) const
{
  return (itsTimeDimensions.find(origintime) != itsTimeDimensions.end());
}

bool TimeDimensions::isValidTime(const Fmi::DateTime& t,
...
bool TimeDimensions::isIdentical(const TimeDimensions& td) const
{
  for (const auto& item : itsTimeDimensions)
  {
    ...
    const std::shared_ptr<TimeDimension>& tdim1 = item.second;
    const std::shared_ptr<TimeDimension>& tdim2 = td.itsTimeDimensions.at(item.first);
...
std::ostream& operator<<(std::ostream& ost, const TimeDimensions& timeDimensions)
{
  try
  {
    ...
    for (const auto& ot : origintimes)
    {
      ...
      const TimeDimension* td = &timeDimensions.getTimeDimension(ot);
