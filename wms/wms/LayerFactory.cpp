...
LayerType determine_product_type(const Config& wmsconfig,
...
std::string determine_producer(const Config& theConfig, Json::Value& root)
{
  try
  {
...
SharedLayer create_wms_layer(const Config& theConfig, Json::Value& root)
{
  try
  {
                                               ...
                                               const Config& theConfig)
{
  try
  {
    ...
    if (!json.isNull())
    {
      ...
      layer->title->init(json, theConfig.itsDaliConfig);
    }
    ...
    if (!json.isNull())
    {
      ...
      layer->abstract->init(json, theConfig.itsDaliConfig);
    }
    ...
    layer->refs = theConfig.supportedWMSReferences();
                                                           ...
                                                           const Config& theConfig)
{
  try
  {
    ...
    Spine::JSON::preprocess(root, root_dir, customer_layers_dir, theConfig.itsJsonCache);
