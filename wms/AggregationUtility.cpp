...
TS::TimeSeriesGenerator::LocalTimeList get_tlist(const std::list<boost::posix_time::ptime>& qengine_times,
							...
							const boost::optional<TS::ParameterAndFunctions>& param_funcs)
{
  try
  {
	...
	if(do_aggregation)
	{
	  ...
	  auto tlist = get_tlist(*qengine_times, valid_time.utc_time(), interval_behind, interval_ahead);
	  // QEngine query
...
bool set_aggregation_period(Engine::Observation::Settings& settings, const std::vector<TS::ParameterAndFunctions>& paramFuncs)
{
  try
  {
	if(paramFuncs.empty())
	...
	for(const auto& paramAndFunctions : paramFuncs)
	{
	  ...
	  {
		aggregationIntervalBehind = std::max(aggregationIntervalBehind, funcs.innerFunction.getAggregationIntervalBehind());
		aggregationIntervalAhead = std::max(aggregationIntervalAhead, funcs.innerFunction.getAggregationIntervalAhead());	  
	  }
	}
...
bool is_location_parameter(const std::string&paramname)
{
  try
  {
	return (paramname == "stationlon" || paramname == "stationlat" || paramname == "fmisid" || TS::is_location_parameter(paramname));
  }
  catch (...)
  {
    throw Fmi::Exception::Trace(BCP, "Operation failed!");
...
TS::TimeSeriesVectorPtr prepare_data_for_aggregation(const std::string& producer,
...
TS::TimeSeriesVectorPtr do_aggregation(const TS::LocalTimePoolPtr& localTimePool,
...

{
  try
  {
	...
	// Get timesries grouped by fmisd and make sure that all timesteps in tlist are in the result set (some timesteps in some locations may contain None-value)
	...
	TS::TimeSeriesVectorPtr ret = boost::make_shared<TS::TimeSeriesVector>(raw_data->size(), TS::TimeSeries(settings.localTimePool));
    ...
    for (const auto& observation_result_location : result_by_location)
	{
	  ...
	  TS::TimeSeriesVectorPtr prepared_data = prepare_data_for_aggregation(settings.stationtype,
																		   settings.parameters,
																		   observation_result,
																		   parameterResultIndexes);

	  // Do aggregation
	  TS::TimeSeriesVectorPtr aggregated_data = do_aggregation(settings.localTimePool,
															   paramFuncs,
															   prepared_data,
															   parameterResultIndexes);
