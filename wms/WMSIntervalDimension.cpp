...
WMSIntervalDimension::WMSIntervalDimension(int interval_start, int interval_end, bool interval_default) 
{
...
std::string WMSIntervalDimension::getDefaultInterval() const
{
  ...
  if(!itsIntervals.empty())
	{
	  const auto& first_item = itsIntervals.front();
		return (Fmi::to_string(first_item.interval_start) + "," + Fmi::to_string(first_item.interval_end));
	}
