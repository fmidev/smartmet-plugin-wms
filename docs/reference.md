...
The six examples below are drawn from the regression suite (`test/dali/customers/test/products/map_*.json`) and double as reference outputs. All six render the same Northern-Baltic view (lon 18°–27°, lat 59°–65°, 900×600 px) of the high-resolution `gshhg.gshhs_h_l1` table (GSHHG-h L1 land polygons, ≈ 200 m vertex spacing, ~14 000 polygons in the bbox prefilter). They are directly comparable so that the visual effect of each option is easy to read. The three amalgamation rows (`map_amalgamate`, `map_amalgamate_wide`, `map_amalgamate_extreme`) form a deliberate calibration sequence — `amalgamation_length` 0.01 → 0.02 → 0.05 — showing how the silhouette degrades as the gap distance grows past a sensible value for the rendering pixel scale.
