...
The five examples below are drawn from the regression suite (`test/dali/customers/test/products/map_*.json`) and double as reference outputs. All five render the same Northern-Baltic view (lon 18°–27°, lat 59°–65°, 900×600 px) so that they are directly comparable.

| Description                                                                                                                                                                          | Map JSON snippet                                                                                                                                              |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `map_baseline.json` — no simplification. Reference image showing the full source resolution (≈ 4 080 vertices for the four-country selection).                                       | `{ "schema": "natural_earth", "table": "admin_0_countries", "where": "iso_a2 IN ('FI','SE','AX','EE')" }`                                                     |
| `map_simplifier_dp.json` — Douglas-Peucker @ 3 px (~76 % vertex reduction). Spiky/saw-toothed coastline; instructive but not the recommended choice for natural features. | `{ ..., "simplifier": "douglas_peucker", "tolerance": 3.0 }`                                                                                                  |
| `map_simplifier_vw.json` — Visvalingam-Whyatt @ 5 px (~72 % reduction). Blockier than the baseline but preserves the overall shape character; the recommended default for natural coastlines. | `{ ..., "simplifier": "visvalingam_whyatt", "tolerance": 5.0 }`                                                                                               |
| `map_amalgamate.json` — amalgamation only, `amalgamation_length=0.05`, `amalgamation_area=0.0005` (≈ 3 km gap, ≈ 1.5 km² minimum island). Archipelago islands merge into chunkier outlines while distinct islands and the mainland coast are kept. | `{ ..., "amalgamation_length": 0.05, "amalgamation_area": 0.0005 }`                                                                                            |
| `map_amalgamate_simplified.json` — same amalgamation followed by Visvalingam-Whyatt @ 1.5 px. The recommended pipeline for archipelago / dense-coastline rendering.                  | `{ ..., "amalgamation_length": 0.05, "amalgamation_area": 0.0005, "simplifier": "visvalingam_whyatt", "tolerance": 1.5 }`                                     |
