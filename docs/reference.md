...
The four examples below are drawn from the regression suite (`test/dali/customers/test/products/map_*.json`) and double as reference outputs. All four render the same Northern-Baltic view (lon 18°–27°, lat 59°–65°, 900×600 px) of the high-resolution `gshhg.gshhs_h_l1` table (GSHHG-h L1 land polygons, ≈ 200 m vertex spacing, ~14 000 polygons in the bbox prefilter). They are directly comparable so that the visual effect of each option is easy to read.
...
| `map_amalgamate.json` — amalgamation, `amalgamation_length=0.01` (≈ 0.6 km gap), `amalgamation_mainland_area=1000` km², `minarea=2` km². Nearby skerries merge into landmasses that survive `minarea`; the mainland bypasses triangulation and is emitted unchanged. The archipelago character is preserved rather than dissolved as it is in the simplifier-only cases. | `{ ..., "amalgamation_length": 0.01, "amalgamation_mainland_area": 1000, "minarea": 2 }`                                                                  |
| `map_amalgamate_mainland.json` — same as above but with `amalgamation_mainland_amalgamate=true`. Each mainland polygon is run through the CDT by itself so its own bays close up below the gap-triangle threshold, producing a visibly more solid coastline. Costs ≈ 220 ms extra vs the bypass-only variant on this test. | `{ ..., "amalgamation_length": 0.01, "amalgamation_mainland_area": 1000, "amalgamation_mainland_amalgamate": true, "minarea": 2 }`                       |
| `map_amalgamate_simplified.json` — same amalgamation + `minarea` followed by Visvalingam-Whyatt @ 1.5 px. The recommended pipeline for archipelago / dense-coastline rendering. | `{ ..., "amalgamation_length": 0.01, "amalgamation_mainland_area": 1000, "minarea": 2, "simplifier": "visvalingam_whyatt", "tolerance": 1.5 }`                                                    |
