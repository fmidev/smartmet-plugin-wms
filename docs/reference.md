...
| `map_amalgamate.json` — amalgamation, `amalgamation_length=0.02` (≈ 1.2 km gap), `minarea=2` km² (~77 % vertex reduction, ≈ 10 500 remaining). Nearby skerries merge into landmasses that survive `minarea`; the archipelago character is preserved rather than dissolved as it is in the simplifier-only cases. | `{ ..., "amalgamation_length": 0.02, "minarea": 2 }`                                                                                                          |
| `map_amalgamate_simplified.json` — same amalgamation + `minarea` followed by Visvalingam-Whyatt @ 1.5 px (~92 % total reduction, ≈ 3 400 remaining). The recommended pipeline for archipelago / dense-coastline rendering. | `{ ..., "amalgamation_length": 0.02, "minarea": 2, "simplifier": "visvalingam_whyatt", "tolerance": 1.5 }`                                                    |
