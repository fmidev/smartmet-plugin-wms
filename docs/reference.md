...
- [Examples](#examples)
  - [Map Projections](examples/maps.md)
  - [Dali Examples](examples/dali.md)
  - [WMS Examples](examples/wms.md)
  - [WMTS Examples](examples/wmts.md)
  - [OGC Tiles Examples](examples/tiles.md)
...
<tr><td>subdivide</td><td>int</td><td>0</td><td>Bilinear cell subdivision for smoother isoband boundaries. 0 (default) keeps the classic linear marching-squares straight segment between each pair of cell edge intersections. N in [2..10] splits every interior level-curve segment into N chords with N-1 samples on the true bilinear level curve. N=1 is accepted as a no-op. Most visible when individual grid cells are large relative to screen pixels and have high corner-value contrast (e.g. a single high radar pixel surrounded by zeros turns from a sharp diamond into a rounded four-lobed blob). Forwarded to the contour engine and further to <a href="https://github.com/fmidev/smartmet-library-trax">trax</a>.</td></tr>
<tr><td>subdivide_min_cell_pixels</td><td>double</td><td>2.0</td><td>Automatic sub-pixel gate on <code>subdivide</code>. When &gt;0, the effective subdivision count is forced to 0 whenever the projected data cells are smaller than this many output pixels across their shorter dimension; interior bilinear samples would not be visible at that scale. Estimated from an approximate median of a 5x5 uniform sub-lattice of the projected coordinate matrix. Set to 0 to disable the gate entirely and always honour the <code>subdivide</code> value.</td></tr>
</table>
...
| subdivide\_min\_cell\_pixels | double          | 2.0           | Automatic sub-pixel gate on `subdivide`. Effective subdivision is forced to 0 when projected data cells are smaller than this many output pixels. Set to 0 to disable.                                                                        |
...
The placement algorithm — multi-angle local-extremum scan, divisibility-weighted MST selection, data-probed orientation — is documented in [isolabel_algorithms.md](isolabel_algorithms.md), with a gallery of rendered tests showing each tunable in action.
...
| amalgamation_length | double  | -             | Maximum gap length the polygon amalgamator may bridge to merge nearby polygons (e.g. archipelago islands). **In CRS coordinate units** — for `EPSG:4326` that means decimal degrees, not metres or kilometres. At 60° N, 0.05° ≈ 3 km. See [Map amalgamation and simplification](#map-amalgamation-and-simplification). |
...
| simplifier          | string  | -             | Polygon/line simplification algorithm: `douglas_peucker` (distance-based vertex removal) or `visvalingam_whyatt` (area-based vertex removal).                                                                |
| tolerance           | double  | -             | Simplification tolerance in **pixels**. Converted to CRS coordinate units using the projection box before being applied. For Douglas-Peucker this is the perpendicular distance threshold; for Visvalingam-Whyatt the area threshold of the squared pixel size. Typical useful values are 1–3 px. |
...
`tolerance` is specified in output pixels and converted to CRS coordinate units once per request using the active projection. Typical values are around 1–3 pixels: small enough that the silhouette is preserved at the chosen resolution, large enough that ~50–75 % of redundant vertices are removed.

> **Caution on Douglas-Peucker for natural coastlines.** DP keeps the points furthest from the chord between consecutive anchors, which on a smoothly-curving coastline tends to keep the tips of peninsulas while straight-cutting the bays between them. The result can look spiky/jagged at higher tolerances. Visvalingam-Whyatt removes the smallest-area triangles first and tracks the overall shape character better — prefer it for coastlines, rivers, and other smooth natural features. DP remains the right choice for polygonal/man-made features (administrative borders, building footprints) and when a strict "max perpendicular error" guarantee is needed.
...
| `simplifier=douglas_peucker`    | Distance-based vertex removal: drops a vertex if its perpendicular distance to the chord between its neighbours is below tolerance. Tolerance is in **pixels** at the active projection.                                                    | Polygonal/man-made features (administrative borders, building footprints) and any case where you want a strict "no point moves more than N pixels" guarantee.                                            |
| `simplifier=visvalingam_whyatt` | Area-based vertex removal: repeatedly removes the vertex whose triangle with its neighbours has the smallest area. Tolerance is in **pixels²**.                                                                                              | Smooth natural features (coastlines, rivers, lake outlines). Preserves the overall shape character better than DP, at slightly higher cost. The recommended default for natural-earth-style coastlines. |
...
- Douglas-Peucker tolerance is a max perpendicular error in pixels; Visvalingam-Whyatt tolerance is roughly an area in pixel². Equivalent visual aggressiveness usually corresponds to similar numeric values, but a side-by-side comparison at the target zoom is the only reliable way to pick.
...
The five examples below are drawn from the regression suite (`test/dali/customers/test/products/map_*.json`) and double as reference outputs. All five render the same Northern-Baltic view (lon 18°–27°, lat 59°–65°, 900×600 px) of the high-resolution `gshhg.gshhs_h_l1` table (GSHHG-h L1 land polygons, ≈ 200 m vertex spacing, ~14 000 polygons in the bbox prefilter). They are directly comparable so that the visual effect of each option is easy to read.
...
| `map_simplifier_dp.json` — Douglas-Peucker @ 3 px + `minarea=2` km² (~90 % vertex reduction, ≈ 4 700 remaining). Spiky/saw-toothed coastline; instructive but not the recommended choice for natural features. Without amalgamation, the archipelago skerries individually fail the area filter and are lost. | `{ ..., "simplifier": "douglas_peucker", "tolerance": 3.0, "minarea": 2 }`                                                                                    |
| `map_simplifier_vw.json` — Visvalingam-Whyatt @ 5 px + `minarea=2` km² (~89 % reduction, ≈ 5 200 remaining). Blockier than the baseline but preserves the overall shape character; the recommended default for natural coastlines. | `{ ..., "simplifier": "visvalingam_whyatt", "tolerance": 5.0, "minarea": 2 }`                                                                                 |
| `map_amalgamate.json` — amalgamation, `amalgamation_length=0.01` (≈ 0.6 km gap), `minarea=2` km² (~58 % vertex reduction, ≈ 19 100 remaining). Nearby skerries merge into landmasses that survive `minarea`; the archipelago character is preserved rather than dissolved as it is in the simplifier-only cases. | `{ ..., "amalgamation_length": 0.01, "minarea": 2 }`                                                                                                          |
| `map_amalgamate_simplified.json` — same amalgamation + `minarea` followed by Visvalingam-Whyatt @ 1.5 px (~90 % total reduction, ≈ 4 500 remaining). The recommended pipeline for archipelago / dense-coastline rendering. | `{ ..., "amalgamation_length": 0.01, "minarea": 2, "simplifier": "visvalingam_whyatt", "tolerance": 1.5 }`                                                    |
...
| Document | Contents |
|----------|----------|
| [Map Projections](examples/maps.md) | All PROJ-supported projections tested via the `/dali` `map` product |
| [Dali Examples](examples/dali.md) | All `/dali` endpoint test cases: output formats, isobands, isolabels, symbols, wind, METAR, Hovmoeller, fire weather, world projections, and more |
| [WMS Examples](examples/wms.md) | `/wms` GetMap, GetCapabilities, GetFeatureInfo, and GetLegendGraphic test cases |
| [WMTS Examples](examples/wmts.md) | `/wmts` GetCapabilities, GetTile (isoband and temperature numbers) |
| [OGC Tiles Examples](examples/tiles.md) | `/tiles` GetCollections and GetTile test cases |
